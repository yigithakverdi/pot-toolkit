...
static inline void process_ingress_packet(struct rte_mbuf* mbuf, uint16_t rx_port_id) {
  ...
  case RTE_ETHER_TYPE_IPV6: {
    ...
    case 0: {
      ...
      size_t min_ingress_size = sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv6_hdr) + actual_srh_size +
    ...
    case 1: {
      ...
      struct rte_ether_addr mac = {{0x02, 0x56, 0xe6, 0xd5, 0x57, 0x05}};
      send_packet_to(mac, mbuf, 0);
      break;
    }
    case 2: {
      LOG_MAIN(DEBUG, "Processing packet with SRH-only (no PoT).\n");

      add_srh_only_header(mbuf);
      ...
      char dst_ip_str[INET6_ADDRSTRLEN];
      inet_ntop(AF_INET6, &segments[next_sid_index], dst_ip_str, sizeof(dst_ip_str));
      LOG_MAIN(DEBUG, "SRH-only mode: Forwarding to first segment: %s\n", dst_ip_str);
      ...
      if (next_mac) {
        ...
        LOG_MAIN(DEBUG, "Packet sent to next hop\n");
      } else {
        LOG_MAIN(ERR, "No MAC found for next SID, dropping packet\n");
        rte_pktmbuf_free(mbuf);
      }
...
void process_ingress(struct rte_mbuf** pkts, uint16_t nb_rx, uint16_t rx_port_id) {
  ...
  //   dropped: %" PRIu64 ", RX errors: %" PRIu64 ", TX errors: %" PRIu64,
